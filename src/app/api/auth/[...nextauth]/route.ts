...
// Got the client id and client secret id and auth secret id is provided by us as a random inside of .env.local file.
...
// We can create these handler function by overself or export using any external library if installed.
...
// In newer versions we have to configure file using the given provided above as new vers only alow nextAuth usage in client comps.

// Extending Session interface to specify the id so that typescript doesnt give error when we build the app.
// We are extending interface by declaring "next-auth" module,  we dont need this for User interface as it has the id defined.
// To check there type simply log the user object and then find the items and based on that create the interface.
// Type "any" can run in development not but not in production.
